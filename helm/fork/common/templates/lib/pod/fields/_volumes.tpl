...
*/ -}}
{{- define "common.lib.pod.field.volumes" -}}
  ...
  {{- range $identifier, $persistenceValues := $persistenceItemsToProcess -}}
    ...
    {{- if eq (default "persistentVolumeClaim" $persistenceValues.type) "persistentVolumeClaim" -}}
      {{- $pvcName := (include "common.lib.chart.names.fullname" $rootContext) -}}
      {{- if $persistenceValues.existingClaim -}}
        {{- /* Always prefer an existingClaim if that is set */ -}}
        {{- $pvcName = tpl $persistenceValues.existingClaim  $rootContext -}}
      {{- else -}}
        {{- /* Otherwise refer to the PVC name */ -}}
        {{- $object := (include "common.lib.pvc.getByIdentifier" (dict "rootContext" $rootContext "id" $identifier) | fromYaml) -}}
    ...
    {{- else if eq $persistenceValues.type "configMap" -}}
      ...
      {{- else if $persistenceValues.identifier -}}
        {{- $object := (include "common.lib.configMap.getByIdentifier" (dict "rootContext" $rootContext "id" $persistenceValues.identifier) | fromYaml ) -}}
        {{- if not $object -}}
    ...
    {{- else if eq $persistenceValues.type "secret" -}}
      ...
      {{- if $persistenceValues.name -}}
        {{- $objectName = tpl $persistenceValues.name $rootContext -}}
      {{- else if $persistenceValues.identifier -}}
        {{- $object := (include "common.lib.secret.getByIdentifier" (dict "rootContext" $rootContext "id" $persistenceValues.identifier) | fromYaml ) -}}
        {{- if not $object -}}
    ...
    {{- else if and (ge ($rootContext.Capabilities.KubeVersion.Minor | int) 33) (eq $persistenceValues.type "image") -}}
      ...
      {{- else -}}
        {{- $_ := set $volume.image "reference" (include "common.lib.imageSpecificationToImage" (dict "rootContext" $rootContext "imageSpec" $persistenceValues.image)) -}}
      {{- end -}}
