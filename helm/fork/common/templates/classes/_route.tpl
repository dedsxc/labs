    ...
    - backendRefs:
      ...
      {{- else }}
        {{- range .backendRefs }}
          ...
          {{- else if .identifier }}
            {{- $service = (include "common.lib.service.getByIdentifier" (dict "rootContext" $rootContext "id" .identifier) | fromYaml ) }}
            {{- if not $service }}
      ...
      - group: {{ .group | default "" | quote}}
        ...
        weight: {{ include "common.lib.defaultKeepNonNullValue" (dict "value" .weight "default" 1) }}
        {{- end }}
      {{- end }}
