...
spec:
  ...
  {{- $serviceName := include "common.lib.chart.names.fullname" $rootContext }}
  {{- with (dig "statefulset" "serviceName" nil $statefulsetObject) }}
    {{- if kindIs "map" . }}
      {{- $serviceName = (include "common.lib.service.getByIdentifier" (dict "rootContext" $rootContext "id" .identifier) | fromYaml ).name }}
    {{- else }}
