            ...
            backend:
              service:
                ...
                {{ else if .service.identifier -}}
                  {{ $service = (include "common.lib.service.getByIdentifier" (dict "rootContext" $rootContext "id" .service.identifier) | fromYaml ) -}}
                  {{ if not $service -}}
                ...
                {{ if empty (dig "port" nil .service) -}}
                  ...
                  {{ if $service -}}
                    {{ $defaultServicePort := include "common.lib.service.primaryPort" (dict "rootContext" $rootContext "serviceObject" $service) | fromYaml -}}
                ...
                {{ else -}}
                  ...
                  {{ else if kindIs "string" .service.port -}}
                    ...
                    {{ $servicePort = include "common.lib.service.getPortNumberByName" (dict "rootContext" $rootContext "serviceID" .service.identifier "portName" .service.port) -}}
                  {{ end -}}
                {{ end -}}
