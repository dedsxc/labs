...
{{- define "common.render.secrets" -}}
  ...
  {{- range $identifier := keys $enabledSecrets -}}
    ...
    {{- $secretObject := (include "common.lib.secret.getByIdentifier" (dict "rootContext" $rootContext "id" $identifier) | fromYaml) -}}

    {{- /* Include the Secret class */ -}}
    {{- include "common.class.secret" (dict "rootContext" $rootContext "object" $secretObject) | nindent 0 -}}
  {{- end -}}
{{- end -}}
...
*/}}
{{- define "common.render.secrets.fromFolder" -}}
  {{- $rootContext := $ -}}
  ...
  {{- if $secretsFromFolderEnabled -}}
    ...
    {{- include "common.lib.secret.fromFolder.validate" (dict "rootContext" $ "basePath" ($secretsFromFolder.basePath | default "" )) -}}
    ...
    {{- range $folder, $entry := $collected -}}
      ...
      {{- end -}}
      {{- $secretObject := (include "common.lib.valuesToObject" (dict "rootContext" $rootContext "id" $folder "values" $secretValues) | fromYaml) -}}
