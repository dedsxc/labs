...
*/}}
{{- define "umbrella.lib.controller.volumes" -}}
    ...
    {{- else -}}
      ...
      {{- if $persistence.nameOverride -}}
        {{- if not (eq $persistence.nameOverride "-") -}}
          {{- $pvcName = (printf "%s-%s" (include "umbrella.lib.chart.names.fullname" $) $persistence.nameOverride) -}}
        {{- end -}}
      {{- else -}}
        {{- $pvcName = (printf "%s-%s" (include "umbrella.lib.chart.names.fullname" $) $index) -}}
      {{- end -}}
    {{- end }}
