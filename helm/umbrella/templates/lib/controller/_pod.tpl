...
*/ -}}
{{- define "umbrella.lib.controller.pod" -}}
imagePullSecrets:
...
initContainers:
    ...
    {{- range $index, $key := (keys .Values.initContainers | uniq | sortAlpha) }}
      ...
      {{- if $container.env -}}
        ...
        {{- $newEnv := fromYaml (include "umbrella.lib.container.envVars" $) -}}
  ...
  {{- end }}
containers:
  {{- include "umbrella.lib.controller.mainContainer" . | nindent 2 }}
  ...
  {{- end }}
  {{- with (include "umbrella.lib.controller.volumes" . | trim) }}
volumes:
    {{- nindent 2 . }}
  {{- end }}
...
topologySpreadConstraints:
    ...
    {{- range $constraint := . }}
      {{- if not $constraint.labelSelector -}}
        {{- $_ := set $constraint "labelSelector" dict }}
      {{- end }}
      {{- if not $constraint.labelSelector.matchLabels -}}
        {{- $_ := set $constraint.labelSelector "matchLabels" (include "umbrella.lib.metadata.selectorLabels" $ | fromYaml) }}
      {{- end }}
      {{- $topologySpreadConstraints = append $topologySpreadConstraints $constraint }}
    {{- end }}
    {{- tpl (toYaml $topologySpreadConstraints) $ | nindent 2 }}
  {{- end }}
