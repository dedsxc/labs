{{/*
This template serves as a blueprint for argo rollout service objects that are created
within the common library.
*/}}
{{- define "umbrella.class.argoRolloutService" -}}
...
{{- $enabledPorts := include "umbrella.lib.service.enabledPorts" (dict "serviceName" $serviceName "values" $values) | fromYaml }}
{{- $primaryPort := get $values.ports (include "umbrella.lib.service.primaryPort" (dict "values" $values)) }}
---
apiVersion: v1
kind: Service
metadata:
  name: {{ $serviceName }}-rollout
  labels:
  ...
  {{- range $name, $port := $enabledPorts }}
    - port: {{ $port.port }}
      ...
      nodePort: {{ $port.nodePort }}
        {{ end }}
      {{- end -}}
  {{- with (merge ($values.extraSelectorLabels | default dict) (include "umbrella.lib.metadata.selectorLabels" . | fromYaml)) }}
